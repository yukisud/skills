...
### codebase-memory-mcp(コード知識グラフ / ローカル完結)
コードベースを知識グラフ化し、AIが「ファイルを1個ずつ読む」より大幅に少ないトークンで構造を把握できるMCP。**処理は100%ローカルでコードは外部に出ない**(DeusData製・MIT・単一バイナリ)。`tool-maintenance-ja` の解析や大規模改修の下地になる。
1. **導入前**: `skill-vetting` で監査(OSSでも毎回。特にinstall.shの中身と、ネットワーク送信が本当に無いかを確認してから実行)
2. インストール(パッケージマネージャ経由を推奨。`curl | bash` は中身を読んでから):
```bash
# npm/PyPI/Homebrew等で入る。インストール後、対象リポジトリでインデックス構築
```
3. Claude Code に MCP登録(`claude mcp add` で起動コマンドを登録)
4. 検証: 小さめのリポジトリでインデックス構築→「この関数の呼び出し元」等の1クエリが返ること
- ハマりどころ: 巨大リポジトリの初回インデックスは時間がかかる/生成された知識グラフDBは `.gitignore` へ(コミット不要)/**ローカル完結なので認証情報は不要**——逆に「クラウド同期」を促すオプションが出たら有効化しない(コードが外に出る)
